...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.rect = self.image.get_rect(center=(x, y))
        # Ajuste da hitbox: agora será 1/4 da largura original e corretamente alinhada
    ...
    def draw(self, surface):
        surface.blit(self.image, self.rect.topleft)
        # Remover a linha de colisão que desenha a hitbox
...
def main():
    ...
    while True:
        ...
        if game_over:
            show_game_over()
            pygame.time.wait(3000)  # Aguardar 3 segundos antes de fechar ou reiniciar
            break
